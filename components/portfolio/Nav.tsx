...
export default function Nav() {
  ...
  return (
    <>
      ...
      <header className="fixed top-[max(0.875rem,env(safe-area-inset-top))] inset-x-0 z-50 flex items-center justify-end px-4 sm:px-6 md:hidden pointer-events-none">
        {/* Right: Iconic H Menu Button (Slightly larger with premium glass bezel) */}
        <button
          aria-label="Toggle Menu"
          onClick={() => setOpen(!open)}
          className="pointer-events-auto p-1.5 rounded-[18px] border border-white/24 bg-[#0a0a0e]/90 shadow-[0_12px_30px_-5px_rgba(0,0,0,0.9),inset_0_1.5px_1px_rgba(255,255,255,0.45)] transition-all duration-300 active:scale-90 hover:scale-105 touch-manipulation"
          style={{
      ...
      >
        ...
        >
          ...
          {/* Contact Button (Liquid Vermilion Glass Pill - Links to /contact Page) */}
          <Link
            href="/contact"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-x-3.5 top-[max(4.25rem,calc(env(safe-area-inset-top)+3.25rem))] z-40 rounded-3xl p-5 md:hidden flex flex-col gap-2.5 border border-white/18 bg-[#0a0a0e]/95 shadow-[0_25px_60px_-10px_rgba(0,0,0,0.9),inset_0_1.5px_1px_rgba(255,255,255,0.35)] max-h-[calc(100dvh-5.5rem)] overflow-y-auto overscroll-contain"
            style={{
