...
import { ArrowUpRight, X, Sparkles, Film } from "lucide-react";
    ...
    <>
      <section id="work" className="relative z-10 py-24 md:py-36">
        <div className="max-w-[1400px] mx-auto px-5 md:px-8">
          ...
          {/* Heading */}
          <div className="flex flex-col xl:flex-row xl:items-stretch justify-between gap-6 lg:gap-10 mb-16 md:mb-24">
            <h2 className="leading-[1.08] sm:leading-[1.04] tracking-[-0.03em] font-black text-[28px] xs:text-[34px] sm:text-[46px] md:text-[58px] lg:text-[72px] xl:text-[84px]">
              <span className="block gradient-text pb-1 sm:pb-2 whitespace-nowrap">
                Frames I couldn’t forget,
              </span>
              <span className="block gradient-text font-serif-editorial italic pb-1 sm:pb-2 whitespace-nowrap">
                moments made to stay.
              </span>
            </h2>
            <div className="max-w-md lg:max-w-[480px] xl:max-w-[520px] flex flex-col justify-center">
              <p className="text-white/65 text-[16px] sm:text-[17px] md:text-[18px] lg:text-[19px] xl:text-[20px] leading-[1.55] tracking-[0.02em] font-normal">
                A collection of narrative shorts, commercials, and visual journeys I’ve brought to life behind the camera. Step in and see the world through my lens.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-16 md:gap-y-20">
            {featuredProjects.map((p, i) => {
              const isBehance = p.type === "behance" || (p.stills && p.stills.length > 0);
              return (
                <div
                  key={p.id}
                      ...
                      <div className="flex items-end justify-between gap-4 py-3 transition-all duration-300 group-hover:pl-2">
                        ...
                        <div className="shrink-0 flex items-center gap-2 text-[11px] tracking-[0.2em] uppercase text-white/70 group-hover:text-white transition-colors">
                          <span className="hidden sm:inline">
                            {isBehance ? "VIEW CASE STUDY" : "VIEW PROJECT"}
                          </span>
          ...
          <AnimatePresence>
            {selectedProject && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[999999] bg-black/85 backdrop-blur-xl md:backdrop-blur-2xl flex items-center justify-center p-3 sm:p-5 md:p-8 overscroll-contain"
                onClick={() => setSelectedProject(null)}
              >
                {selectedProject.type === "behance" || (selectedProject.stills && selectedProject.stills.length > 0) ? (
                  <motion.div
                    initial={{ scale: 0.95, opacity: 0, y: 14 }}
                    animate={{ scale: 1, opacity: 1, y: 0 }}
                    exit={{ scale: 0.95, opacity: 0, y: 14 }}
                    onClick={(e) => e.stopPropagation()}
                    className="relative w-full max-w-5xl lg:max-w-6xl xl:max-w-[1360px] bg-[#070709] rounded-2xl sm:rounded-3xl overflow-hidden border border-white/15 max-h-[94vh] max-h-[94dvh] overflow-y-auto overscroll-contain shadow-[0_35px_120px_rgba(0,0,0,0.95)]"
                  >
                    <BehanceProjectView
                  ...
                  <motion.div
                    ...
                    onClick={(e) => e.stopPropagation()}
                    className="relative w-full max-w-5xl lg:max-w-6xl xl:max-w-[1220px] bg-[#0c0c0e]/95 backdrop-blur-2xl rounded-2xl sm:rounded-3xl overflow-hidden border border-white/15 p-5 sm:p-7 md:p-9 flex flex-col gap-5 max-h-[94vh] max-h-[94dvh] overflow-y-auto overscroll-contain shadow-[0_35px_120px_rgba(0,0,0,0.95)]"
                  >
                    <button
                      onClick={() => setSelectedProject(null)}
                      className="absolute top-4 right-4 z-40 w-10 h-10 rounded-full border border-white/20 grid place-items-center text-white bg-black/70 hover:bg-[var(--accent)] transition-colors cursor-pointer touch-manipulation active:scale-95 shadow-lg"
                      aria-label="Close"
                    >
                      <X size={18} />
                    </button>
